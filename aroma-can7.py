...
def cli(block,cleanup,threads,workingdir,sourcedir,allblocks,memory,error,force,update):
    ...
    if update == 1:
        get_serieslist(sourcedir,logger)
    ...
    def runPipeline(seriesname):
        ...
        # if local_exists(seriespath,"segments,fracb.tab"):
        #     print('Skip Series ' + seriesname + ' as it has been processed. \n')
        # else:
...
def get_serieslist(sourcedir,logger):
    with open(os.path.join(sourcedir,'series_platform.tsv'), 'w',encoding='utf-8') as wfile:
        metapath = "/Volumes/arraymapIncoming/GEOmeta/"
        serlist = os.listdir(metapath)
        serlist = [i for i in serlist if i.startswith('GSE') == 1]
        wfile.write('Series\tPlatform\n')
        for series in serlist:
            platform = []
            wholepath = os.path.join(metapath,series,'geometa.soft')
            try:
                with open(wholepath,encoding='utf-8', errors='ignore') as f:
                    content = f.readlines()
                    content = [x.strip() for x in content]
                    for line in content:
                        if line.startswith("!Series_platform_id = GPL"): platform.append(line.split(" = ")[1])
                for t in zip([series]*len(platform),platform):
                    line = '\t'.join(x for x in t)
                    wfile.write(line + '\n')
            except Exception as ex:
                template = "{0}\t{1}\t{2}"
                message = template.format(series,type(ex).__name__, ex.args)
                logger.debug(message)
        wfile.close()

    with open(os.path.join(sourcedir,'series_platform.tsv'), 'r',encoding='utf-8') as f:
        content = f.readlines()
    #    subf = []
    s = set()
    with open(os.path.join(sourcedir,'affy.tsv'),'w',encoding='utf-8') as wf:
        for line in content:
            line = line.strip()
            if line.split('\t')[1] in ['GPL3718','GPL3720','GPL6801','GPL6804','GPL2004','GPL2005','GPL16131','GPL18637','GPL2641']:
                s.add(line.split('\t')[0])
        wf.write('\n'.join(s))
        wf.close()
